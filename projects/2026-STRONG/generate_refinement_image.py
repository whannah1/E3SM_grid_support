...
# cmd += f' -composite -blur 0x10 -fuzz 90% -fill white -opaque white {fig_file}.tmp.png'
# print(cmd)

# fig_file=2026-STRONG-CA-RRM_refinement_image_v1.png
# magick convert ${fig_file} \( +clone -blur 0x20 -fuzz 90% -fill gray -opaque white \)  -composite -blur 0x10 -fuzz 50% -fill white -opaque white ${fig_file}.tmp.png
...
for state in shpreader.Reader(states_shp).records():
    ...
    if (state.attributes.get('admin') == 'United States of America' and 
        state.attributes.get('name') in 'California'):
