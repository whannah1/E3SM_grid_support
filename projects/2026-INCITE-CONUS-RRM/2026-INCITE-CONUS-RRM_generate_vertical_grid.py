...
print_end_summary = False

nsmooth = 40

grid_support_root = os.getenv('HOME')+f'/E3SM_grid_support'
output_root = f'{grid_support_root}/2026-INCITE-CONUS-RRM'
...
def main():
    ...
    # ofile = f'{output_root}/2026-INCITE-CONUS-RRM_L128_v1_c20251211.nc'
    # dk_list = [ 5, 5, 5, 5,  5,  5,  5,  5, 88,]
    # dz_list = [20,40,60,80,100,125,150,200,220,]
    # nsmooth = 20
    # fix_lowest_spacing = True

    # ofile = f'{output_root}/2026-INCITE-CONUS-RRM_L128_v2_c20251211.nc'
    # dk_list = [ 5, 5,  5,105,  8]
    # dz_list = [20,50,100,200,800]
    # nsmooth = 20
    # fix_lowest_spacing = True

    # ofile = f'{output_root}/2026-INCITE-CONUS-RRM_L128_v3_c20251211.nc'
    # dk_list = [ 6, 6, 6,100,  4,  4,   2]
    # dz_list = [20,40,80,200,400,800,1600]
    # nsmooth = 20
    # fix_lowest_spacing = True

    expected_nlev = 128+16
    ofile = f'{output_root}/2026-INCITE-CONUS-RRM_L144_v1_c20251211.nc'
    dk_list = [10,10,10,100,  6,  6,   2]
    dz_list = [20,40,80,200,400,800,1600]
    nsmooth = 40
    fix_lowest_spacing = True
    ...
    if 'dz_list' in locals() and 'zlev' not in locals():
        ...
        for d,dk in enumerate(dk_list):
            for k in range(dk):
                ...
                kk += 1
    #---------------------------------------------------------------------------
    # use this to check basic level stats
    if sum(dk_list)!=expected_nlev:
        print()
        print(f'{tclr.RED}ERROR - levels do not add to {expected_nlev}{tclr.END}')
        print()
        print(f'num zlev: {sum(dk_list)}')
        print(f'max zlev: {max(zlev)}')
        print()
        exit()
    #---------------------------------------------------------------------------
    ...
    # print(); print(zlev_smoothed[1])
    # # address kink at second mid-point from the surface
    # if fix_lowest_spacing:
    #     zlev = zlev_smoothed
    #     zmid = np.array([ ((zlev[k]+zlev[k+1])/2.) for k in range(num_mlev) ])
    #     zmid[1] = ( zmid[0] + zmid[2] )/2.
    #     zlev[1] = ( zmid[0] + zmid[1] )/2.
    #     zlev_smoothed = zlev
    #     # for k in range(smth_k_beg,smth_k_beg+1):
    #     #     # zlev_smoothed[k] = ( 0.25*zs_tmp[k-1] + 0.5*zs_tmp[k] + 0.25*zs_tmp[k+1] )
    #     #     zlev_smoothed[k] = ( 0.5*zs_tmp[k-1] + 0.5*zs_tmp[k+1] )
    #     #     # zlev_smoothed[k] = ( 0.25*zs_tmp[k-1] + 0.75*zs_tmp[k] )
    #     # zlev_smoothed[2] = ( 0.5*zs_tmp[k-1] + 0.5*zs_tmp[k+1] )
    # print(); print(zlev_smoothed[1])
    # # exit()
    ...
    if print_int_debug:
        for k in range(num_ilev): 
            k2 = num_ilev-k-1
            dz = zlev[k2] - zlev[k2-1] if k2>0 else 0
            print(f'{k:03}  ({k2:02})    {ilev[k]:8.2f}    {zlev[k2]:8.1f}    {dz:8.1f}')
    ...
    ofile_tmp = ofile.replace(f'{grid_support_root}/','')
    
    print(); print(f'  {tclr.CYAN}{ofile}{tclr.END}')
    print(); print(f'  {tclr.CYAN}{ofile_tmp}{tclr.END}')
    print()
...
#-------------------------------------------------------------------------------
    ...
    print('-'*80)
    print()
    print(f'{tclr.GREEN}Successfully created vertical grid file{tclr.END}')
    print()
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
