#!/bin/bash
#-------------------------------------------------------------------------------
export swag_slurm_mail_user=hannah6@llnl.gov
...
swag_scratch_nersc=$SCRATCH
if [ -e ${swag_scratch_lcrc}  ]; then swag_host=LCRC; fi
if [ -e ${swag_scratch_alcf}  ]; then swag_host=ALCF; fi
if [ -e ${swag_scratch_olcf}  ]; then swag_host=OLCF; fi
if [ -e ${swag_scratch_nersc} ]; then swag_host=NERSC; fi
#-------------------------------------------------------------------------------
if [ -z "$swag_host" ]; then echo -e ${RED}ERROR - host machine not identified ; exit 1 ; fi
#-------------------------------------------------------------------------------
